...
namespace ARDrone.Input
{
    public class JoystickInput : DirectInputInput
    {
        ...
        public static List<GenericInput> GetNewInputDevices(IntPtr windowHandle, List<GenericInput> currentDevices)
        {
			...
			List<DeviceInstance> gameControllerList = (List<DeviceInstance>)di.GetDevices(DeviceClass.GameController, DeviceEnumerationFlags.AttachedOnly);

		    for (int i = 0; i < gameControllerList.Count; i++)
            {
                
				
				//Device device = (Device)Joy;
				Joystick Joy = null;
                Device device = null;
                try
                {
					Joy = new Joystick(di, gameControllerList[i].InstanceGuid);
					device = (Device)Joy;
                }
                catch (Exception)
                { }

				if (device != null && device.Information.ProductGuid != new Guid("0306057e-0000-0000-0000-504944564944") &&       // Wiimotes are excluded
					!CheckIfDirectInputDeviceExists(gameControllerList[i], currentDevices))
                {
        ...
        public override List<String> GetPressedButtons()
        {
            ...
            try
            {
				...
				Joystick joyx = (Joystick)device;
				JoystickState state = joyx.GetCurrentState();
        ...
        public override Dictionary<String, float> GetAxisValues()
        {
            ...
            try
            {
                ...
                axisValues[Axis.Axis_R.ToString()] = GetFloatValue(state.RotationZ);
				axisValues[Axis.Axis_Rx.ToString()] = GetFloatValue(state.RotationX);
				axisValues[Axis.Axis_Ry.ToString()] = GetFloatValue(state.RotationY);

				axisValues[Axis.Axis_POV_1.ToString()] = CalculatePOVValue(state.GetPointOfViewControllers()[0]);
