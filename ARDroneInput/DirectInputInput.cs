...
namespace ARDrone.Input
{
    public abstract class DirectInputInput : ButtonBasedInput
    {
		...
		protected static bool CheckIfDirectInputDeviceExists(DeviceInstance device, List<GenericInput> currentDevices)
        {
            for (int i = 0; i < currentDevices.Count; i++)
            {
				if (device.InstanceGuid.ToString() == currentDevices[i].DeviceInstanceId)
                    return true;
            }
            return false;
        }
        ...
        {
			device.SetCooperativeLevel(windowHandle, CooperativeLevel.Nonexclusive | CooperativeLevel.Background);
        ...
        public override String DeviceInstanceId
        {
            get
            {
				...
				else { return device.Information.InstanceGuid.ToString(); }
            }
        }
    }
}
